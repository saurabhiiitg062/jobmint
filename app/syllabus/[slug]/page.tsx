...
export async function generateStaticParams() {
  try {
    const data = await api.getJobs({ category: 'Syllabus', limit: 100 });
    return data.jobs.map((job: Job) => ({
      slug: job.slug,
    }));
  } catch (error) {
    console.warn('API error in generateStaticParams for syllabus:', error);
    return mockJobs.filter(j => j.category === 'Syllabus').map((job) => ({
      slug: job.slug,
    }));
  }
}
