...
export async function generateStaticParams() {
  try {
    const data = await api.getJobs({ category: 'Cutoff', limit: 100 });
    return data.jobs.map((job: Job) => ({
      slug: job.slug,
    }));
  } catch (error) {
    console.warn('API error in generateStaticParams for cutoffs:', error);
    return [];
  }
}
...
export default async function CutoffSlugPage({ params }: PageProps) {
  const { slug } = await params;
  ...
  try {
    ...
    console.warn('API error fetching cutoff slug, using mock fallbacks.');
  }
